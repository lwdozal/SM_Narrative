...
def next_post():
	...
	try:
		...
		# nex = driver.find_element(By.CSS_SELECTOR,"div[class='_abm0']") #gets previous too
...
def img_src_alt(hashtag, img_cnt, img_call):
	...
	# print("img_sourceAll",img_source)
	# img_source_first = soup.find(attrs={"class":'_aagv'}).find('img')['src']
	# img_source_first = img_source.next_element
	# print("img_source_first",img_source_first)
	# img_source_first = img_source_first('img')['src']
	WebDriverWait(driver, 3)
...
def vid_src_alt(hashtag, img_cnt, img_call):
	...
	# download_img(hashtag, vid_src, img_cnt, img_call)
	# vid_src = 'https://www.instagram.com/reel/' + img_call + '/?utm_source=ig_web_copy_link'
	# mp4 = urllib.request.urlretrieve(vid_src, hashtag + '01/' + img_call + '_{}.mp4'.format(img_cnt))
	# thumbnail_alt = driver.find_element(By.CSS_SELECTO, 'div[class = "_aagv"] > img').get_attribute("alt") #img
...
####################
    ### testing with instascrape
	#use instascrape's Reel module to identify the reel of the particular image id
	# reel_vid = Reel('https://www.instagram.com/reel/' + img_call + '/?utm_source=ig_web_copy_link')
	# print("reel_vid", reel_vid)
	# # #use instareel scrape function to get the reel
	# reel_vid.scrape(headers=False)
	# # #download reel to path
	# reel_vid.download(fp=f"."+hashtag + '01/' + img_call + '_{}.mp4'.format(img_cnt))
	# print("Downloaded video-reel")
####################
...
def image_info(hashtag, img_cnt, img_call):
	if video_check() != None:
		src, alt, account, likes, comment, time  = vid_src_alt(hashtag, img_cnt, img_call)
	else: 
		src, alt, account, likes, comment, time = img_src_alt(hashtag, img_cnt, img_call)
	# else:
	# 	print("did not find anything in post")
	
	return src, alt, account, likes, comment, time
...
def all_first_images():
	...
	# imgs = {src for src in soup.findAll('div', class_="_aagv")}
	# i=0
	# time.sleep(5)
	# for img in imgs:
	# 	i += 1
	...
	# 	jpg = urllib.request.urlretrieve(img_src, hashtag + '_sweep/thumbnail_' + img_call + '_{}.jpg'.format(str(i)))

	# # 	# download_img(hashtag, img_src, img_cnt, img_call + str(i))
	# # 	print("img_src in for loop", img_src)
	# # 	img_srcs.append(img_src)

	# 	img_alt = img.find('img')['alt']
	# 	# images_alt.append(img_alt)
	# 	print("img_alt in for loop", img_alt)
		
	# 	s0 = pd.Series(jpg[0], name='first_image')
	# 	s1 = pd.Series(hashtag + '/'+ img_call + ".jpg", name='file_path')
